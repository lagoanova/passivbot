...
class Telegram:
    def __init__(self, config: dict, bot, loop):
        ...
        self.loop = loop
        self.config = config
        self._chat_id = config['chat_id'] if 'chat_id' in config['chat_id'] else os.environ['CHAT_ID']
        self._updater = Updater(token=config['token'] if 'chat_id' in config['token'] else os.environ['TOKEN'])
